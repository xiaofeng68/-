(function (D) {
    var E = D.env, doc = document, docurl = doc.location.href, share = null, lookup = [], ci = E.compId, loaded = false, oms = ci >= 20000000,
        ua = navigator.userAgent.toLowerCase(), id_s = 'looyu_id', id_l = 'looyu_' + ci, localdb, localtemp, homepage = oms && 'http://www.looyuoms.com' || 'http://www.talk99.cn',
        dct = [], sametime = 120, fpath = E.file + '/../default/', conhtml, site = E.subComp || 0,
        winopt = 'height=450,width=650,directories=no,location=no,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no,top=100,left=200',
        winH = window.innerHeight, htmlEle = document.getElementsByTagName('html')[0],
    ...
    var securityPath = function (p) {
        if (!E.secure || p.indexOf("http://static.soperson") == -1) return p;
    ...
    var have = doc.compareDocumentPosition ? function (a, b) {
        ...
        if (browser.cacheswf) path += '?_t=' + +new Date();
        var os = tpl(str, {path: path, id: id});
        sharediv.innerHTML = '<iframe src="about:blank" id="shareWrapper"></iframe>';
        show(sharediv);
        try {
            var h = '<html><head><script type="text/javascript">'
                + 'var domReady=false,doyoo={};doyoo.probe=function(){return parent.doyoo.probe();};'
                + 'doyoo.init=function(o){return parent.doyoo.init(o);};'
                + 'doyoo.domReady=function(){return domReady;};'
                + 'doyoo.shareReady=function(){parent.doyoo.shareReady();};'
                + 'window.onload=function(){domReady=true};'
                + 'window.findSWF=function(id,ie){return ie?window[id]:document[id];};'
                + '</script></head><body>'
                + '<script type="text/javascript">document.write(\'' + os + '\')</script></body></html>';
        ...
        // return browser.msie ? window[id] : doc[id];
        ...
        var u = url + '&x=' + (+new Date()),
            ...
            s = createEle('script', {type: "text/javascript", src: u}, h);
        ...
        if (v) {
            ...
            if (currentStyle(v).display === 'none') v.style.display = 'block';
        }
    }, hide = function (v) {
        if (typeof v == 'string') v = $(v);
        if (v) {
        ...
        while ((attr = re.exec(e)) != null) {
            var at = attr[0];
            if (t.addEventListener) t.addEventListener(at, f, false);
            else if (t.attachEvent) t.attachEvent('on' + at, f);
            else  t['on' + at] = f;
        }
        ...
        if (E.reseve && E.reseve != 'null') obj.r = E.reseve;
        obj._d = +new Date();
        return obj2str(obj);
    }, chatURL = function (ch) {
        if (!E.uId) return '#';
        ...
        if (!ch || typeof ch == 'object') {
            ...
            if (ch && (ch.sId || ch.g)) {
                ...
                if (cg && cg != '0') ot.push('g=' + cg);
                md = 5;
            }
        } else if (typeof ch == 'string' && ch) {
            ot.push(ch);
            md = 2;
        }
        ot.push('md=' + md);
        ...
        try {
            ...
            if (ch.force || $.mobile) window.location = url;
        }
        return false;
        ...
        show(o);
        var w = parseInt(o.offsetWidth || 500), h = parseInt(o.offsetHeight || 300),
            pw = getBody().clientWidth, ph = getBody().clientHeight,
            sp = [0 - h + fixTop(), sLeft() + pw, ph + fixTop(), 0 - w + sLeft()],
            rp = ['top', 'left'];
        var sv = sp[pos], d = pos % 2, tv = d ? (tar.left || (pw - tar.right - w)) : (tar.top || (ph - tar.bottom - h)),
            td = d ? 'left' : 'top';
        var setP = function (v) {
            o.style[td] = v + 'px';
        };
        ...
        if (h.length > 100) h = h.substring(0, 100);
        return {
            c: 'a', i: ci, v: E.vId, u: E.uId, p: E.pId || '', ref: E.refer, site: site,
            h: h, w: u, scn: (screen.width + '*' + screen.height),
            t: this.type || '', ct: E.counter, r: E.reseve || '', p0: E.p0 || docurl
        };
    };
    D.util = {
        openChat: openChat,
    ...
    extend(flier.prototype, {
        ...
        drag: function (e) {
            ...
            if (mv.x) o.style.left = e.clientX - this.offs.x + 'px';
            if (mv.y) o.style.top = e.clientY - this.offs.y + fixTop() + 'px';
        },
        drop: function (e) {
    ...
    var M = function (cfg) {
        ...
        if (!this.obj) {
            this.obj = createEle('div', {id: 'doyoo_monitor'});
        }
    ...
    var P = function (cfg) {
        ...
        if (!this.obj) {
            this.obj = createEle('div', {id: 'doyoo_panel'});
        }
        extend(this, cfg);
    };
    var Mask = function () {
    ...
    extend(DP.prototype, {
        defUI: function () {
            ...
            this.obj.innerHTML =
                '<div class="looyu_p_tl looyu_p_t"><div class="looyu_p_tr looyu_p_t"><div class="looyu_p_tc looyu_p_t"><div id="looyu_p_h">X</div>&nbsp;</div></div></div>'
                + '<div class="looyu_p_ml looyu_p_m"><div class="looyu_p_mr  looyu_p_m"><div class="looyu_p_mc"><div id="looyu_p_c"></div></div></div></div>'
                + '<div class="looyu_p_bl  looyu_p_b"><div class="looyu_p_br  looyu_p_b"><div class="looyu_p_bc  looyu_p_b">&nbsp;</div></div></div>';
    ...
    extend(M.prototype, {
        defUI: function (mark) {
            ...
            if ($.mobile && !this.preferConfig) {
                ...
                inh = '<div id="doyoo_mon_innner" style="position:relative;width:100%;height:100%;-webkit-transform: translateZ(0px)">' +
                    '<div id="doyoo_mon_main"></div>' +
                    '<a id="doyoo_mon_accept" class="doyoo_mon_btn" href="javascript:;" onclick="doyoo.util.openChat();doyoo.util.accept();return false;">' + (accs[lang] || 'CHAT') + '</a>' +
                    '<div id="doyoo_mon_closer"></div></div>';
                extend(this, {width: 200, height: 120});
                o.innerHTML = inh;

            } else if (i <= 8 && i > 0 || i > 100) {
                try {
                    inh = '<div id="doyoo_mon_innner" style="position:relative;width:100%;height:100%;-webkit-transform: translateZ(0px)"><div id="doyoo_mon_head"></div><div id="doyoo_mon_main"></div>' +
                        '<div id="doyoo_mon_foot"><a id="doyoo_mon_accept" class="doyoo_mon_btn" href="javascript:;" onclick="doyoo.util.openChat();doyoo.util.accept();return false;"></a>' +
                        '<a id="doyoo_mon_phone" class="doyoo_mon_btn"  href="javascript:;" onclick="doyoo.util.openPhone();doyoo.util.accept();return false;"></a>' +
                        '<div id="doyoo_mon_refuse" class="doyoo_mon_btn"></div></div>' +
                        '</div><div id="doyoo_mon_closer"></div></div></div>';
                    o.innerHTML = inh;
                } catch (e) {
                ...
                inh = '<div id="doyoo_mon_inner" style="position:relative;width:100%;height:100%;-webkit-transform: translateZ(0px)">'
                    + '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="{mw}" height="{mh}">'
                    + '<param name="movie" value="{mbg}"/><param name="wmode" value="transparent"/><embed src="{mbg}"  width="{mw}" height="{mh}" type="application/x-shockwave-flash" wmode="transparent"></embed></object>'
                    + '<div id="doyoo_mon_head" class="doyoo_mon_head" style="position:absolute;left:{mtl}px;top:{mtt}px;width:{mtw}px;height:{mth}px"></div>'
                    + '<a id="doyoo_mon_accept" href="javascript:;" onclick="doyoo.util.openChat();doyoo.util.accept();return false;" style="position:absolute;margin-left:5px;display:block;left:{mbal}px;bottom:{mbab}px;width:{mbaw}px;height:{mbah}px;background:url({mbabg}) no-repeat;text-align:center"></a>'
                    + '<a id="doyoo_mon_phone" href="javascript:;" onclick="doyoo.util.openPhone();doyoo.util.accept();return false;" style="position:absolute;display:block;margin:0;left:{mbpl}px;bottom:{mbpb}px;width:{mbpw}px;height:{mbph}px;background:url({mbpbg}) no-repeat;text-align:center"></a>'
                    + '<div id="doyoo_mon_refuse"  style="position:absolute;display:block;left:{mbdl}px;bottom:{mbdb}px;width:{mbdw}px;height:{mbdh}px;background:url({mbdbg}) no-repeat;text-align:center;top:auto;right:auto;cursor:pointer"></div>'
                    + '<div id="doyoo_mon_main" class="doyoo_mon_main"  style="position:absolute;left:{mml}px;top:{mmt}px;width:{mmw}px;height:{mmh}px"></div></div>';
                inh2 = '<div id="doyoo_mon_inner" style="position:relative;width:100%;height:100%;background-image:url({mbg});-webkit-transform: translateZ(0px)">'
                    + '<div id="doyoo_mon_head" class="doyoo_mon_head"  style="position:absolute;left:{mtl}px;top:{mtt}px;width:{mtw}px;height:{mth}px"  ></div>'
                    + '<a id="doyoo_mon_accept" href="javascript:;" onclick="doyoo.util.openChat();doyoo.util.accept();return false;" style="position:absolute;margin-left:5px;display:block;left:{mbal}px;bottom:{mbab}px;width:{mbaw}px;height:{mbah}px;background:url({mbabg}) no-repeat"></a>'
                    + '<a id="doyoo_mon_phone" href="javascript:;" onclick="doyoo.util.openPhone();doyoo.util.accept();return false;" style="position:absolute;margin:0;display:block;left:{mbpl}px;bottom:{mbpb}px;width:{mbpw}px;height:{mbph}px;background:url({mbpbg}) no-repeat"></a>'
                    + '<div id="doyoo_mon_refuse" href="javascript:;" style="position:absolute;display:block;left:{mbdl}px;bottom:{mbdb}px;width:{mbdw}px;height:{mbdh}px;background:url({mbdbg}) no-repeat;top:auto;right:auto;cursor:pointer"></div>'
                    + '<div id="doyoo_mon_main" class="doyoo_mon_main"   style="position:absolute;left:{mml}px;top:{mmt}px;width:{mmw}px;height:{mmh}px"></div></div>';
                if (i > 0) {
        ...
        buildMain: function (txt) {
            txt = txt || this.text;
            this.main.innerHTML = /^https?:\/\/.*/.test(txt) ? ('<ifr' + 'ame frameborder="0" width="100%" height="100%" src="' + txt + '"></ifr' + 'ame>') : txt;
        },
        builder: function () {
            var o = this.obj;
            if (typeof beforeLooyuMonitorBuilder == 'function') beforeLooyuMonitorBuilder.apply(this);
            else {
                this.defUI(true);
            }
            ...
            //createEle('iframe',{id:'doyoo_leave',src:'about:blank'}, this.obj);
        ...
        pump: function (r) {
            ...
            switch (r.c) {
                case 'a':
                    ...
                    var t = r.t && (+r.t) || 10;
                    t < 3 && (t = 3);
                    if (t && !this.timer) this.timer = window.setInterval(this.driver.doyoodg(this), t * 1000);
                ...
                case 'e':
                    var es = r.es;
                    for (var v in es) (es.hasOwnProperty(v) && typeof es[v] == 'object' && es.hasOwnProperty(v)) && this.doEvent(es[v]);
                    break;
                case 'ac':
                case 'de':
                    break;
        ...
        force: (function () {
            ...
            var buildOldPage = function (mon, t) {
                ...
                if (mon.mini == 1) {
                    var s = (function () {
                        ...
                        if (browser.msie) browser.strict && html.push(fc.text || '');
                        else html.push('<!DOCTYPE html PUBLIC "', fc.publicId, '" "', fc.systemId, '">');
            ...
            var buildForceUrl = function (me, param) {
                var talkUrl = chatURL(param || this.chat);
                var found = false;
            ...
            var showEarlyMessage = function (msg, me) {
                ...
                if (!earlyPanel) {
                    earlyPanel = createEle('div', {id: 'doyoo_early'});
                    earlyPanel.innerHTML = '<div class="doyoo_early_wrap">' +
                        '<div class="doyoo_early_head">' +
                        '<div class="doyoo_early_photo">&nbsp;</div> ' +
                        '<a href="javascript:;" onclick="closeDoyooEarly();return false;">&nbsp;</a> </div>' +
                        '<div class="doyoo_early_msg">' +
                        '<div id="doyoo_early_from"></div><div id="doyoo_early_detail"></div>' +
                        '</div>' +
                        '<div class="doyoo_early_arrow"></div><div class="doyoo_early_arrow_inner"></div></div> ';
                    earlyPanel.style.width = me.miniWidth + 'px';
                ...
                earlyPanel.style.bottom = (parseInt(fChat.style.bottom) + 60 ) + 'px';
                show(earlyPanel);
            };

            var createChatFrame = function (me) {
                var fChat = createEle('div', {id: 'doyoo_f_chat'});
                ...
                fChat.innerHTML = '<div id="doyoo_f_head"><div id="doyoo_f_close"></div>' +
                    '<div id="doyoo_f_min"></div><div id="doyoo_f_title">&nbsp;</div></div>'
                    + '<div  id="doyoo_f_main"></div>';
                ...
                attachE($('doyoo_f_close'), 'click dblclick', function () {
                    if (me.minimize) {
                ...
                attachE(window, 'message', function (evt) {
                    ...
                    if (msg) {
                        ...
                        } else if (msg.type == 'chatBegin') {
                            me.isChating = true;
                        } else if (msg.type == 'chatEnd') {
                            me.isChating = false;
                        } else if (msg.type == 'chatFocus' || msg.type == 'chatBlur') {

                        } else if (msg.type == 'chatMinimize') {
                            me.minForce();
                        } else if (msg.type == 'chatClose') {
                            fChat.style[$.mobile ? 'left' : 'right'] = '-100000px';
                        }
                    }
                });
            ...
            return function (ch, auto, mobileClick) {
                // this.hideDiv(0);
                msgp && msgp.minimize();
                ...
                var t = this.mini, talkUrl = buildForceUrl(this, ch);
                if (auto) {
                    talkUrl += '&autoChat=1';
                }
                ...
                var fChat = $('doyoo_f_chat') || createChatFrame(this);
                var fMain = $('doyoo_f_main');
                this.minimize = $.mobile && !mobileClick;

                if ($.mobile) {
                    fChat.style.left = '0px';
                    fChat.style.bottom = '0px';
                    fChat.style.width = "100%";
                    fChat.style.height = mobileClick ? '100%' : '50px';
                    fChat.style.position = 'fixed';
        ...
        minForce: function () {
            var me = this;
            var fChat = $('doyoo_f_chat'), fMain = $('doyoo_f_main');
            var f = $('doyoo_f_work');
            try {
                var contentWin = f.contentWindow;
            ...
            if (!$.mobile) {
                ...
                fChat.style.top = m.minimize ? 0 : 'auto';
                fChat.style.bottom = 0;
                fChat.style.position = browser.iPhone && m.minimize && 'absolute' || 'fixed';
                fChat.style.height = browser.iPhone && m.minimize && 'auto' || (m.minimize ? '100%' : '50px');
                fMain.style.height = m.minimize ? '100%' : '250px';
                $('doyoo_f_title').style.background = 'transparent';
                doc.body.style.marginBottom = '50px';
                if (browser.iPhone) {
                    if (m.minimize) {
                        setTimeout(function () {
    ...
    extend(P.prototype, {
        ...
        builder: function () {
            ...
            if (typeof beforeLooyuPanelBuilder == 'function') beforeLooyuPanelBuilder.apply(this);
            else {
                this.defUI();
            }
            if (this.position != -1) {
        ...
        winBuilder: function () {
            ...
            if (this.mode == 2) {
                ...
                var hcont = ['<div id="doyoo_panel_main">'], groups = this.customers.groups,
            ...
            if (this.mode == 1) {
                o.innerHTML = '<div id="ohead"><div id="ocls" ></div><div id="omin" onclick=""></div>' +
                    '<div id="otitle"></div><div id="oentrance"></div></div><div id="ocontent"></div>' +
                    '<div id="ofoot"><div onclick=javascript:window.open("' + homepage + '")></div></div>';
        ...
        buildList: function () {
            ...
            for (var v = 0; v < depts.length; v++) {
                var dept = depts[v], t = m && ((dept.mode == 2) && 'g' || 'gw') || 'd',
        ...
        iconBuilder: function () {
            ...
            var w = this.collapse && this.collapsed && this.collapseW
                || this.collapse && !this.collapsed && (this.width + this.collapseW)
                || this.width;
            ...
            if (this.collapse) {
                ...
                var collapseTpl = '<a href="javascript:;" class="talk99_collapse" id="{collapseId}"' +
                    ' style="width:{collapseW}px;height:{collapseH}px;margin-top:{collapseT}px;float:{collapseFloat};background-image:url({collapseImage})" ' +
    ...
    extend(MsgPanel.prototype, (function () {
        ...
        var lang_tip = {
            ...
            lang_name = {sc: '\u59d3\u540d\uff1a', tc: '\u59d3\u540d\uff1a', en: 'name', ru: '\u0444.\u0438.\u043e'},
            lang_phone = {
                sc: '\u7535\u8bdd\uff1a',
                tc: '\u96fb\u8a71\uff1a',
                en: 'phone',
        ...
        return {
            ...
            defUI: function () {
                var h = '<div class="t99_mpanel_inner">'
                    + '<div class="t99_mpanel_title" id="t99_mpanel_title">{title}</div><div id="t99_mpanel_miner"></div>'
                    + '<div class="t99_mpanel_main"><div class="t99_mpanel_content"><textarea id="t99_msg_content">{txtTip}</textarea></div>'
                    + '<div class="t99_mpanel_info"><span class="t99_mpanel_name">{txtName}</span><input type="text" id="t99_msg_name"></div>'
                    + '<div class="t99_mpanel_info"><span class="t99_mpanel_phone">{txtPhone}</span><input type="text" id="t99_msg_phone"></div>'
                    + '<div class="t99_mpanel_info"><span class="t99_mpanel_email">{txtEmail}</span><input type="text" id="t99_msg_email"></div>'
                    + '<div class="t99_mpanel_info"><span class="t99_mpanel_qq">{txtQQ}</span><input type="text" id="t99_msg_qq"></div>'
                    + '</div><div id="t99_mpanel_submit" >{txtSubmit}</div>'
                    + '<a id="t99_mpanel_freePhone" href="javascript:;" onclick="doyoo.util.openPhone();return false;">{txtFreePhone}</a>'
                    + '</div>';
                var idx = this.index;
    ...
    var parseEnv = function (o) {
        ...
        //alert(ct);
        var ev = {
            uId: o.id || localdb.match(/^([^_]+)/) || E.vId, vId: o.vid && parsev() || getl('v') || ++ct && E.vId,
            counter: ct, refer: (!refer || sameSite(refer)) ? getl('ref') : refer, p0: getl('p0') || docurl,
    ...
    D.start = function () {
        ...
        if (topd == window || !findm) {
            ...
            D.monParam && ready(m.record.doyoodg(m));
        } else {
    ...
    ready(function () {
        ...
        if (snf) {
            var ids = snf.ids.split(','), lb = 'looyu_bound', gids = snf.gids.split(','),
            ...
            var ebound = function (nd, op) {
                if (nd.nodeType == 3)return op.getAttribute(lb);
            ...
            var snifferNode = function (nd) {
                ...
                if (ebound(nd, o_p)) return;
                if (nd.nodeType == 3 && o_p.nodeName != 'A') {
                    if (nd.data.search(reg) == -1) return;
